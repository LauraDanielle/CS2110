...
public class HW2Bases
{
	...
	public static int strdtoi(String decimal)
	{
		...
		int output = 0;
		for (int num = 0; num <= decimal.length(); num++ )
		{
			output = output + (int) (Math.pow(10, num)*(((decimal.charAt(num)-'0'))));
	...
	public static int strxtoi(String hex)
	{
		...
		for (int num = 0; num <= hex.length(); num++ ) {
                    if (hex.charAt(num) > 64) {

			bit = hex.charAt(num)-'7' | bit;
                    } else {
